...
export default function PhraseDetailPage() {
  ...
  const phraseId = Number(searchParams.get("phraseId") || 0)

  // デバッグ情報
  useEffect(() => {
    console.log(`PhraseDetail page loaded with categoryId: ${categoryId}, phraseId: ${phraseId}`)
  }, [categoryId, phraseId])
  ...
  const navigateToNextPhrase = () => {
    router.push(`/phraseDetail?categoryId=${categoryId}&phraseId=${phraseId + 1}`)
  }
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 container py-8">
          <div className="flex justify-center py-12">
            <p className="text-red-500">カテゴリーIDまたはフレーズIDが指定されていません</p>
          </div>
        </main>
      </div>
    )
